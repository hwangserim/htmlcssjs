<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
	<head>
		<link href="/springframework/resources/bootstrap-5.3.3-dist/css/bootstrap.min.css" rel="stylesheet">
		<script src="/springframework/resources/bootstrap-5.3.3-dist/js/bootstrap.bundle.min.js"></script>
		<meta charset="UTF-8">
		<title>Insert title here</title>
	</head>
	
	
	<body>
		<div class="card">
  			<div class="card-header">index</div>
  			<div class="card-body">
  			<ul>
  			<li><a href="javascript/exam01_whereto">item</a></li>  	
  			<li><a href="javascript/exam02_variables">variables</a></li>
  			<li><a href="javascript/exam03_operations">operations</a></li>
  			<li><a href="javascript/exam04_function">function</a></li>
  			<li><a href="javascript/exam05_object">object</a></li>
  			<li><a href="javascript/exam06_dynamic_field_method">dynamic</a></li>
  			<li><a href="javascript/exam07_getter_setter">exam07_getter_setter</a></li>
  			<li><a href="javascript/exam08_function_constructor">exam08_function_constructor</a></li>
  			<li><a href="javascript/exam09_class">exam09_class</a></li>
  			<li><a href="javascript/exam10_static">exam10_static</a></li>
  			<li><a href="javascript/exam11_inheritance">exam11_inheritance</a></li>	
  			<li><a href="javascript/exam12_destructuring_assignment">exam12_destructuring_assignment</a></li>
  			<li><a href="javascript/exam13_string">exam13_string</a></li>
  			<li><a href="javascript/exam14_date">exam14_date</a></li>
  			<li><a href="javascript/exam15_math">exam15_math</a></li>
  			<li><a href="javascript/exam16_global_fun">exam16_global_fun</a></li>
  			<li><a href="javascript/exam17_json">exam17_json</a></li>
  			<li><a href="javascript/exam18_regexp">exam18_regexp</a></li>
  			<li><a href="javascript/exam19_event">exam19_event</a></li>
  			<li><a href="javascript/exam20_try_catch">exam20_try_catch</a></li>
  			<li><a href="javascript/exam21_array">exam21_array</a></li>				
  			</ul>
  			</div>
		</div>
	</body>
</html>